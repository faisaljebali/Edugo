...
function ProfDetails() {
    const { id } = useParams();
    const profs = useSelector(state => state.cours);
    console.log(profs);
    const dispatch = useDispatch();
    useEffect(() => {
        window.scrollTo(0, 0);
        dispatch(coursActions.getCoachById(id));
    }, []);

    return (
        <div className="container mg-top-50">
            {
             profs.loading && <div className="center_loader">
                            <BeatLoader
                                size={20}
                                margin={10}
                                color={"#f6b120"}
