...
function ProfItem({prof}) {
    ...
    return (
            <div className="single-item-coach">
                <Link className="link-coach-item" to={'coach-'+prof.id}></Link>
                ...
                <div className="prof_content" dangerouslySetInnerHTML={{__html: prof.description && prof.description.replace(regex, '').substring(0, 150).replace(/(<? *script)/gi, 'illegalscript')+'...'}} ></div> 
                </div>
            </div>
    );
